"""Stage 1: real memory/latency/energy measurement (Table 5) for all 4
fine-tuned cells, per docs/AraBit-1.58_IMPLEMENTATION.md sec:3.5.

Not one of the numbered scripts/00-05 - see results/NOTES.md, same reasoning
as the other scripts/stage1_*.py files.

Usage: python -m scripts.stage1_efficiency
"""
...
def main():
    ...
    for front_end, vocab_size in [("bpe", control_tok.get_vocab_size()), ("mate", len(vocabs["word"]))]:
        for precision in ("fp16", "ternary"):
            ...
            state = torch.load(OUT_DIR / f"checkpoint_ft_{cell}.pt", map_location=device, weights_only=True)
            ...
            if front_end == "bpe":
                ...
                words = (all_words * 2)[:32]  # cycle to guarantee enough words
                seq_len = len(words)
            ...
            print(f"  front-end preprocess: {pre_ms_per_tok:.4f} ms/token "
                  f"({'CAMeL' if front_end=='mate' else 'BPE'}, {pre_tokens} tokens)")
            ...
            joules_per_seq = joules / 20 if joules == joules else float("nan")  # nan-safe
            print(f"  energy: {joules_per_seq:.4f} J/sequence (over {wall_s:.2f}s, 20 sequences)"
                  if joules == joules else "  energy: not available (no NVIDIA GPU / pynvml failed)")
