"""Remediation Phase 1.2.4: recompute gate statistics from the corrected
mate_fp16/mate_ternary seed0 checkpoints (results/stage2_corrected) —
identical logic to scripts/stage4_gate_stats.py, since MATE's own
embedding/gate computation (src/model/mate.py) is unchanged by the
output-head fix; only which checkpoint file is loaded differs. The old
results/stage4/gate_stats.csv was computed from checkpoints that no longer
represent the corrected pipeline and should not be cited going forward.

Usage: python -m scripts.remediation1_gate_stats
"""
