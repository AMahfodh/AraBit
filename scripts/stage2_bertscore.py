"""Adds BERTScore-F1 to Stage 2's SAMER quality eval, all 12 checkpoints,
using the generations already saved by scripts/stage2_finetune_and_eval.py.

Usage: python -m scripts.stage2_bertscore
"""
...
def main():
            ...
            with open(OUT_DIR / f"generations_{cell}_seed{seed}.tsv", encoding="utf-8") as f:
                for row in csv.DictReader(f, delimiter="\t"):
                    if not row["generated"].strip():
                        continue  # bert_score can't handle empty strings on this transformers version, see Stage 1 NOTES.md
    ...
    print("\n=== BERTScore-F1 summary (mean +/- std over 3 seeds) ===")
