"""Remediation Phase 1.2.5: re-measure efficiency for MATE cells only
(BPE cells unaffected by the output-head fix, Phase 1.0). Output head grew
from 12,000x512 to 13,699x512 words->BPE-vocab, so P_f/memory shift -
measured with the real harness, not adjusted arithmetically.

Usage: python -m scripts.remediation1_efficiency
"""
...
def main():
    ...
    vocab_size = control_tok.get_vocab_size()  # 13699, shared with bpe cells now
    ...
    for precision in ("fp16", "ternary"):
        ...
        state = torch.load(IN_DIR / f"checkpoint_ft_{cell}_seed0.pt", map_location=device, weights_only=True)
        ...
        model_ms_per_tok = measure_latency_model_only_ms_per_token(model, dummy, prefix_lens, 1, seq_len, device)
    ...
    with open(OUT_DIR / "efficiency_results_mate_only.csv", "w", newline="") as f:
        w = csv.writer(f)
    ...
    print("\n=== Remediation Phase 1 MATE efficiency summary ===")
    for cell, r in results.items():
